...
function fetchSoundDefinitionsLive(): Promise<SoundEntry[]> {
    ...
    https.get(options, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          const defs = (json.sound_definitions || json) as Record<string, unknown>;
          const sounds: SoundEntry[] = [];
          for (const [name, def] of Object.entries(defs)) {
            if (name === 'format_version' || typeof def !== 'object' || !def) { continue; }
            const d = def as Record<string, unknown>;
            const category = (d.category as string) || 'misc';
            const rawSounds = (d.sounds as unknown[]) || [];
...
class SoundBrowserPanel {
  ...
  public static currentPanel: SoundBrowserPanel | undefined;

  private readonly _panel: vscode.WebviewPanel;
  private _disposables: vscode.Disposable[] = [];
  private _sounds: SoundEntry[] | null = null;
  ...
  private constructor(panel: vscode.WebviewPanel, context: vscode.ExtensionContext) {
    ...
    this._loadSounds(context).then(sounds => {
      ...
      this._loadError = (err as Error).message || 'Error desconocido';
      this._trySendData();
    });
  }

  private _trySendData() {
  ...
  private async _loadSounds(context: vscode.ExtensionContext): Promise<SoundEntry[]> {
    const jsonPath = path.join(context.extensionPath, 'sounds.json');
    const sounds = fs.existsSync(jsonPath)
      ? JSON.parse(fs.readFileSync(jsonPath, 'utf8')) as SoundEntry[]
  ...
  private _scanLocalSounds(baseDir: string, dir: string) {
    try {
